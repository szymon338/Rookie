<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Rookie Flow</title>
    <link rel="stylesheet" href="stylereset.css">
    <link rel="stylesheet" href="style.css">
    <link href="https://fonts.googleapis.com/css?family=Roboto:400,900&display=swap" rel="stylesheet">

</head>
<html>
    <body>
        <header>
             <div class="logo"> <a href="/index.html">
              <img src="images/SVG/Asset 3.svg" width="300" height="100" alt="Freelancer.com logo"></a>
              </div>
              <nav>
              <div class="navbutton">
              <a href="https://www.freelancer.com">Post Project</a>
              </div>
              </nav>
        </header>


        
            <div id="hero">
                    <div id="wide">
                        <h1>FIND THE WORLD'S WORK</h1>
                     </div>
                    <div id="narrow"><div class="inputwrapper">
                         <label for="email" class="newline">Sign up with your email</label>
                         <form action="insert.php" method="post"></form>
                         <input type="text" name="email" id="email" placeholder="john@random.com">
                    <div>
                    </form>
                          <a href="/2-select.html" class="mybutton"> Sign Up</a>
                </div>
            </div>
        </div>
    </div>
        

    
<form action="/insert.php" method="post">
    <p>
        <label for="firstName">First Name:</label>
        <input type="text" name="first_name" id="firstName">
    </p>
    <p>
        <label for="lastName">Last Name:</label>
        <input type="text" name="last_name" id="lastName">
    </p>
    <p>
        <label for="emailAddress">Email Address:</label>
        <input type="text" name="email" id="emailAddress">
    </p>
    <input type="submit" value="Submit">
</form>
</body>
</html>



            
          <div class="agency">
        <div class="slide1">
            
             <h1>Unlimited access to jobs from around the globe. </h1>

        </div></div>

        <div class="row">
            <div class="column"><p>This is the text that explains how great we are. Look at us go! Lorem ipsum dolor sit amet consectetur adipisicing elit. Sequi neque dolore numquam! Possimus expedita Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam reiciendis provident fugiat enim mollitia non pariatur maxime earum nihil perspiciatis porro impedit explicabo dolor voluptatum dignissimos eveniet, laudantium esse accusantium voluptatibus nostrum minus vel aperiam. Voluptate illum dolorem necessitatibus quas, facilis dolor ipsa saepe quia quae facere, nostrum nesciunt. Repellat, natus neque! Sequi, praesentium esse asperiores facere tempora inventore, eveniet, autem dolorum optio aperiam reprehenderit. Maxime quos dicta similique laborum? </p></div>
            <div class="column"><p>This is the text that explains how great we are. Look at us go! Lorem ipsum dolor sit amet consectetur adipisicing elit. Sequi neque dolore numquam! Possimus expeditaThis is the text that explains how great we are. Look at us go! Lorem ipsum dolor sit amet consectetur adipisicing elit. Sequi neque dolore numquam! Possimus expedita Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam reiciendis provident fugiat enim mollitia non pariatur maxime earum nihil perspiciatis porro impedit explicabo dolor voluptatum dignissimos eveniet, laudantium esse accusantium voluptatibus nostrum minus vel aperiam. Voluptate illum dolorem necessitatibus quas, facilis dolor ipsa saepe quia quae facere, nostrum nesciunt. Repellat, natus neque! Sequi, praesentium esse asperiores facere tempora inventore, eveniet, autem dolorum optio aperiam reprehenderit. Maxime quos dicta similique laborum?   </p></div>
          </div>
              
          <div class="row">
            <div class="column"><img src="/images/svg/bird.svg" alt=""></div>
            <div class="column"><h2>Freelancing is great. lorem placeat, delectus quasi eligendi ipsam accusamus distinctio soluta quas voluptas quae nulla? Nam debitis nostrum aliquam, quisquam error ad beatae ipsum quaerat temporibus quia aut ex ut tempora accusantium perferendis? Animi asperiores placeat, eveniet distinctio saepe assumenda ullam accusamus fugit at quia ducimus numquam facere rem aperiam ex ad, velit omnis neque iusto laborum voluptate expedita nam. Est quos porro mollitia commodi dolorem alias aspernatur tempore enim officia, veritatis nisi ipsum reprehenderit vero sequi! Omnis architecto distinctio similique illum, dolorem, vitae soluta eius consectetur magnam voluptatibus ut minus cumque! Possimus totam eos similique ratione vel quidem doloremque magni quia? Deserunt, voluptatum. Quis reiciendis iste, inventore aliquid, deserunt assumenda alias ex doloribus soluta magni in libero, perferendis veritatis? Voluptate accusantium assumenda harum? Accusantium voluptate, illo tempora veniam cumque animi omnis, quod soluta eum rem optio. Architecto provident, voluptatem labore debitis totam corrupti inventore quas. Officiis, laudantium provident aperiam perspiciatis doloribus vel dolores soluta odio, expedita eum quos itaque alias accusantium voluptatem voluptatibus enim nemo cupiditate similique. Molestias facilis odio quae nobis, quisquam aliquam quo perferendis aspernatur commodi vero voluptatem hic impedit. Ex iusto minima dolores esse id sapiente eveniet. Totam, nobis recusandae exercitationem temporibus nam quidem, laborum cupiditate, dolor itaque ullam saepe et in perspiciatis. Quibusdam laboriosam aspernatur omnis! Beatae, incidunt! Tempora quisquam placeat saepe cumque? Quasi mollitia .</h2><div class="slide2"><div></div></div>
          </div>
        
    </div>

    </body>
<footer><footer id="footer" class="footer-1">
    <div class="main-footer widgets-dark typo-light">
    <div class="container">
    <div class="row">
      
    <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget subscribe no-box">
    <h5>Freelancer.com</h5>
    <p>The world's largest freelancing marketplace </p>
    </div>
    </div>
    
    <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget no-box">
    <h5 class="widget-title">Quick Links<span></span></h5>
    <ul class="thumbnail-widget">
    <li>
    <div class="thumb-content"><a href="#.">Stolen Footers</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">Copyright</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">Contact Support</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">International</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">Enterprise</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">Contact Us</a></div>	
    </li>
    <li>
    <div class="thumb-content"><a href="#.">About</a></div>	
    </li>
    </ul>
    </div>
    </div>
    
    <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget no-box">
    <h5 class="widget-title">Get Started<span></span></h5>
    <p>Post your first project</p>
    <a class="btn" href="#." target="_blank">Register Now</a>
    </div>
    </div>
    
    <div class="col-xs-12 col-sm-6 col-md-3">
    
    <div class="widget no-box">
    <h5 class="widget-title">Contact Us<span></span></h5>
    
    <p><a href="mailto:info@domain.com" title="glorythemes">info@domain.com</a></p>
    <ul class="social-footer2">
    <li class=""><a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a></li>
    <li class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a></li>
    <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a></li>
    <li class=""><a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a></li>
    </ul>
    </div>
    </div>
    
    </div>
    </div>
    </div>
      
    <div class="footer-copyright">
    <div class="container">
    <div class="row">
   
    </div>
    </div>
    </div>
    </footer></footer>
</html>